...
public final class Util {
	...
	public static void emitFireLight(Location loc) {
		boolean enabled = ProjectAddons.instance.getCachedBoolean("Properties.Fire.DynamicLight.Enabled", true);
		int brightness = ProjectAddons.instance.getCachedInt("Properties.Fire.DynamicLight.Brightness", 15);
		long keepAlive = ProjectAddons.instance.getCachedLong("Properties.Fire.DynamicLight.KeepAlive", 5000L);
		if (enabled) LightManager.createLight(loc).brightness(brightness).timeUntilFade(keepAlive).build();
	}
