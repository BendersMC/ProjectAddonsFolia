...
public class AnimatedBlock {
	...
	AnimatedBlock(Block block, Queue<AnimationStep> animation, boolean cycle, boolean revert, Queue<Runnable> revertTasks, Particle effect) {
		...
		final BukkitTask[] task = new BukkitTask[1]; // Array to store the reference

		task[0] = Bukkit.getScheduler().runTaskTimer(ProjectAddons.instance, () -> {
			if (!checkStep()) {
				task[0].cancel(); // Access the task from the array
			}
		}, 0L, 1L);
	}
